...
def extract_education(text_lower):
    # Check pursuing/ongoing B.Tech first (common for students)
    if re.search(r'b\.?tech|bachelor of technology|bachelor of engineering', text_lower):
        # Make sure it's not a master's mention
        if not re.search(r'm\.?tech|master of technology', text_lower):
            return 'B.Tech'
    for degree in ['PhD','M.Tech','MBA','M.Sc','B.Tech','B.Sc','Diploma']:
        for kw in EDU_PATTERNS[degree]:
            if kw in text_lower:
                return degree
    # Fallback: generic bachelor/master check
...
def extract_projects_count(text_lower):
    return min(len(re.findall(r'\bproject\b', text_lower)), 10)
...
def score_resume():
    ...
    try:
        ...
        score = score_text(a['found_skills'], a['exp'], a['education'], a['certification'], a['projects_count'], job_role)
        return jsonify({"success":True,"job_role":job_role,"ai_score":score,
            "extracted_experience":a['exp'],"extracted_education":a['education'],
            "found_skills":a['found_skills'],"missing_skills":a['missing_skills'],
            "section_scores":a['section_scores'],"ats_score":a['ats_score'],"ats_hits":a['ats_hits']})
...
def batch_score():
    ...
    for f in files:
        try:
            ...
            results.append({"filename":f.filename,"ai_score":score,"tier":tier,
                "exp":a['exp'],"education":a['education'],"certification":a['certification'],
                "found_skills":a['found_skills'][:8],"missing_skills":a['missing_skills'][:6],
                "ats_score":a['ats_score'],"section_scores":a['section_scores']})
        except Exception as e:
            errors.append({"filename": f.filename, "error": str(e)})
    results.sort(key=lambda x: x['ai_score'], reverse=True)
    for i, r in enumerate(results, 1):
        r['rank'] = i
    return jsonify({"success":True,"job_role":job_role,"total":len(results),"ranked":results,"errors":errors})
